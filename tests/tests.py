...
def test_optional_input_named_access():
    """Missing optional input is accessible by name in run: block without AttributeError."""
    import json
    tmpdir = run(
        dpath("test_optional_input_named_access"),
        shouldfail=False,
        check_results=False,
        cleanup=False,
    )
    assert (tmpdir / "report.json").exists()
    result = json.loads((tmpdir / "report.json").read_text())
    # counts from the present file are merged correctly
    assert result["counts"] == {"a": 3, "b": 1}
    # run: block detected the missing optional file via os.path.exists()
    assert result["missing_optional_detected"] is True
    # the path attribute was accessible (not AttributeError) and points to the expected file
    assert result["missing_optional_path"].endswith("counts_missing.json")
    shutil.rmtree(tmpdir, ignore_errors=ON_WINDOWS)
